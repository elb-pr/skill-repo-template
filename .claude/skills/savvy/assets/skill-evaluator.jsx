...
const EVAL_GENERATOR_SYSTEM = `You are an expert Claude skill evaluator. You receive a complete SKILL.md and your job is to generate a comprehensive set of eval test cases tailored specifically to that skill.

Generate exactly 6 eval test cases covering these REQUIRED categories — do not skip any:

1. PHASE GATE ENFORCEMENT — prompt asks to skip a phase or bypass a prerequisite. Assertions check the skill refuses and cites the relevant constraint verbatim.

2. PHASE 3 STRUCTURE OUTPUT — prompt MUST include: (a) explicit Brief Builder completion statement, (b) all six threshold questions answered with specific answers, (c) request to produce the full SKILL.md. Assertions check XML structure, primacy/middle/recency zone placement, authority vocabulary, positive framing, example count and ordering, constraints_reminder presence. This eval is INVALID if the prompt does not satisfy Phase 1 prerequisites.

3. VALIDATION GATE ENFORCEMENT — prompt asks to package or ship the skill without running validation or without reaching the required pass rate. Assertions check the skill refuses and cites the packaging constraint.

4. PHASE 2 RESEARCH SATURATION — prompt asks to skip research or proceed to Phase 3 with insufficient sources. Assertions check the skill enforces the stopping condition and cannot be rushed.

5. ANTI-PATTERN DIAGNOSIS — prompt describes a skill that has a specific compliance failure (constraint in middle zone, hedging language, inconsistent examples, negative framing — pick one not already used). Assertions check correct root cause identification, correct fix, and reference to validation scripts.

6. CORE PURPOSE — realistic end-to-end prompt testing the skill's primary function. Assertions check the most critical behaviours specific to this skill.

Each eval MUST have specific, falsifiable assertions — never vague ones like "output is helpful" or "response addresses the question". Assertions MUST be checkable from the transcript alone.

CRITICAL: The Phase 3 Structure Output eval (eval 2) MUST set up Phase 1 completion in the prompt itself, otherwise the skill will correctly refuse to generate output and all structure assertions will produce false negatives.
  ...
  "evals": [
    {
      ...
      "prompt": "realistic user prompt that tests this skill",
      "expectations": [
        "specific falsifiable assertion about what the response must contain or do",
...
const EXECUTOR_SYSTEM = `You are executing a test case for a Claude skill. You receive the full SKILL.md and a test prompt. Simulate exactly what Claude would do if this skill were active.

Write a markdown execution report:

## Steps Taken
[Numbered list of every action, tool used, decision made]

## Output Produced
[The actual output — include it in full if it fits, or a detailed summary]

## Files Created
[Files produced, or "None"]

## Skill Instructions Followed
[Explicit Y/N for each major instruction — did you follow it? Why or why not?]

## Notes
[Ambiguities, edge cases, unclear instructions]

Be specific. Quote the skill instructions you are following. Show your work.`;

const GRADER_SYSTEM = `You are a skill eval grader. You receive a test prompt, assertions to check, and an execution transcript. Evaluate each assertion with evidence.

Also critique the assertions — flag any that are trivially satisfied, too vague, or missing important outcomes.

Return ONLY raw JSON, no markdown fences:
{
  "expectations": [
    { "text": "assertion text", "passed": true, "evidence": "specific quote or observation" }
  ],
  "summary": { "passed": 0, "failed": 0, "total": 0, "pass_rate": 0.0 },
  "eval_feedback": {
    "suggestions": [{ "assertion": "weak assertion", "reason": "why weak" }],
    "overall": "brief honest assessment"
  }
}
...
const REPORT_SYSTEM = `You are synthesizing Claude skill eval results and producing an actionable improvement report.

You receive: the skill, all eval prompts, grading results, and pass/fail evidence. Identify patterns across evals — what consistently fails and why.

Write a markdown report:

# Skill Eval Report: {skill_name}

**Overall Pass Rate:** {x}% ({passed}/{total} assertions)

## Headline Assessment
[2-3 sentences — honest overall verdict]

## What Is Working
[Things that consistently pass]

## What Is Failing
[Failure patterns with root causes — not just listing what failed but why]

## Top Improvements
[HIGH/MEDIUM/LOW — specific changes with rationale]

## Eval Quality Notes
[Are the generated evals strong? Any blind spots?]`;

// ── API helpers ───────────────────────────────────────────────────────────────

async function callModel(model, system, userContent, maxTokens = 8000) {
...
async function callJson(model, system, userContent, maxTokens = 8000) {
  const text = await callModel(model, system, userContent, maxTokens);
...
function parseFrontmatter(skillMd) {
  ...
  const name = (skillMd.match(/^name:\s*(.+)$/m) || [])[1]?.trim() || "unknown";
  return { name };
}
...
async function loadSkillZip(file) {
  ...
  if (!skillMdFile) throw new Error("SKILL.md not found");
  const skillMd = await skillMdFile.async("string");
  const { name } = parseFrontmatter(skillMd);
  return { name, skillMd };
}
...
export default function SkillEvaluator() {
  ...
  async function loadSkillFile(file) {
    setLoadError(""); setSkill(null); setEvals([]); setReportMd(""); setError("");
    setLoading(true);
    try { setSkill(await loadSkillZip(file)); }
    catch (e) { setLoadError(e.message); }
    finally { setLoading(false); }
  }

  function updateEval(id, patch) {
    setEvals(prev => prev.map(e => e.id === id ? { ...e, ...patch } : e));
  }

  async function runPipeline() {
    setRunning(true); setError(""); setEvals([]); setReportMd("");

    // ── Opus generates tailored evals
    setStage("Generating evals…");
    let evalsData;
    try {
      const result = await callJson(OPUS, EVAL_GENERATOR_SYSTEM, `SKILL.md:\n\n${skill.skillMd}`, 8000);
      evalsData = result.evals;
      setEvals(evalsData.map(e => ({ ...e, executorMd: null, graderResult: null, status: "idle" })));
    } catch (e) {
      setError("Eval generation failed: " + e.message);
      setRunning(false); setStage(""); return;
    }

    // ── Evals run sequentially
    for (const ev of evalsData) {
      updateEval(ev.id, { status: "running-executor" });
      setStage(`Eval ${ev.id}: executing…`);
      let executorMd;
      try {
        executorMd = await callModel(HAIKU, EXECUTOR_SYSTEM,
          `SKILL.md:\n\n${skill.skillMd}\n\n---\n\nTest prompt:\n${ev.prompt}`, 8000);
        updateEval(ev.id, { executorMd, status: "running-grader" });
      } catch (e) {
        updateEval(ev.id, { status: "error", executorMd: `**Error:** ${e.message}` });
        continue;
      }

      setStage(`Eval ${ev.id}: grading…`);
      try {
        const graderResult = await callJson(HAIKU, GRADER_SYSTEM,
          `SKILL.md:\n\n${skill.skillMd}\n\n---\n\nTest prompt:\n${ev.prompt}\n\nAssertions:\n${ev.expectations.map((a, i) => `${i+1}. ${a}`).join("\n")}\n\nExecution transcript:\n${executorMd}`,
          8000);
        updateEval(ev.id, { graderResult, status: "done" });
      } catch (e) {
        updateEval(ev.id, { status: "error", graderResult: { error: e.message } });
      }
    }

    setStage(""); setRunning(false);
  }

  async function generateReport() {
    setReportRunning(true); setError("");
    try {
      const totalPass  = evals.reduce((s, e) => s + (e.graderResult?.summary?.passed  || 0), 0);
      const totalTotal = evals.reduce((s, e) => s + (e.graderResult?.summary?.total   || 0), 0);
      const evalSummary = evals.map(e => ({ name: e.eval_name, prompt: e.prompt, assertions: e.expectations, grading: e.graderResult }));

      const report = await callModel(OPUS,
        REPORT_SYSTEM.replace("{skill_name}", skill.name),
        `SKILL.md:\n\n${skill.skillMd}\n\nOverall: ${totalPass}/${totalTotal} assertions passed\n\nEval results:\n${JSON.stringify(evalSummary, null, 2)}`,
        8000);
      setReportMd(report);
    } catch (e) {
  ...
  function copyReport() {
    (navigator.clipboard?.writeText(reportMd) ?? Promise.reject()).catch(() => {
      const ta = document.createElement("textarea"); ta.value = reportMd;
      document.body.appendChild(ta); ta.select(); document.execCommand("copy"); document.body.removeChild(ta);
    });
    setCopied(true); setTimeout(() => setCopied(false), 2200);
  }

  const allDone    = evals.length > 0 && evals.every(e => e.status === "done" || e.status === "error");
  const totalPass  = evals.reduce((s, e) => s + (e.graderResult?.summary?.passed || 0), 0);
  const totalTotal = evals.reduce((s, e) => s + (e.graderResult?.summary?.total  || 0), 0);
    ...
    <>
      <style>{`
        ...
        * { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        .summary-bar { background: var(--color-bg-surface); border: 0.5px solid var(--color-border-default); border-radius: var(--radius-interactive); padding: 12px var(--space-4); margin-bottom: var(--space-4); display: flex; align-items: center; justify-content: space-between; }
        .summary-label { font-size: var(--font-size-caption); color: var(--color-text-faint); text-transform: uppercase; letter-spacing: 0.07em; }
        .summary-score { font-size: 22px; font-weight: var(--font-weight-semibold); }
        .summary-score.high { color: var(--color-status-positive-text); }
        .summary-score.mid  { color: var(--color-status-caution-text); }
        .summary-score.low  { color: var(--color-status-negative-text); }
        .summary-detail { font-size: var(--font-size-caption); color: var(--color-text-faint); }
        .eval-list { display: flex; flex-direction: column; gap: var(--space-2); margin-bottom: var(--space-4); }
        .eval-card { background: var(--color-bg-surface); border: 0.5px solid var(--color-border-default); border-radius: var(--radius-interactive); overflow: hidden; transition: border-color var(--duration-normal) var(--ease-out); }
        .eval-card.done { border-color: var(--color-status-positive-border); }
        .eval-card.error { border-color: var(--color-status-negative-border); }
        .eval-card.running-executor, .eval-card.running-grader { border-color: var(--color-status-caution-border); }
        .eval-head { padding: 12px var(--space-4); min-height: 48px; display: flex; justify-content: space-between; align-items: center; cursor: pointer; }
        .eval-name { font-size: var(--font-size-interaction); font-weight: var(--font-weight-medium); color: var(--color-text-secondary); }
        .eval-badge { font-size: var(--font-size-caption); font-weight: var(--font-weight-semibold); padding: 3px 8px 5px; border-radius: 4px; letter-spacing: 0.03em; white-space: nowrap; }
        .eval-badge.idle             { background: var(--color-bg-raised); color: var(--color-text-faint); }
        .eval-badge.running-executor { background: var(--color-status-caution-bg); color: var(--color-status-caution-text); }
        .eval-badge.running-grader   { background: var(--color-status-positive-bg); color: var(--color-status-positive-text); }
        .eval-badge.done             { background: var(--color-status-positive-bg); color: var(--color-status-positive-text); }
        .eval-badge.error            { background: var(--color-status-negative-bg); color: var(--color-status-negative-text); }
        .eval-body { padding: 0 var(--space-4) 12px; border-top: 0.5px solid var(--color-border-default); }
        .eval-prompt { font-size: var(--font-size-caption); color: var(--color-text-muted); margin: var(--space-3) 0 var(--space-2); line-height: var(--line-height-reading); }
        .assert-list { display: flex; flex-direction: column; gap: var(--space-1); margin-bottom: var(--space-3); }
        .assert-row { font-size: var(--font-size-caption); padding: 8px; border-radius: var(--radius-interactive); display: flex; gap: var(--space-2); align-items: flex-start; line-height: var(--line-height-compact); }
        .assert-row.pass { background: #1e2d18; color: var(--color-status-positive-text); }
        .assert-row.fail { background: var(--color-status-negative-bg); color: var(--color-status-negative-text); }
        .assert-icon { flex-shrink: 0; }
        .assert-evidence { font-size: var(--font-size-caption); color: var(--color-text-muted); margin-top: 2px; }
        .eval-feedback { font-size: var(--font-size-caption); color: var(--color-text-muted); margin-top: var(--space-2); font-style: italic; }
        .exec-toggle { font-size: var(--font-size-caption); color: var(--color-border-emphasis); cursor: pointer; margin-top: var(--space-2); user-select: none; }
        .exec-toggle:hover { color: var(--color-text-faint); }
        .exec-block { font-family: 'Courier New', monospace; font-size: var(--font-size-caption); color: var(--color-text-muted); line-height: var(--line-height-reading); white-space: pre-wrap; word-break: break-word; background: var(--color-bg-base); border: 0.5px solid var(--color-border-default); border-radius: var(--radius-interactive); padding: var(--space-3); margin-top: var(--space-2); max-height: 300px; overflow-y: auto; }
        ...
        .report-card { background: var(--color-bg-surface); border: 0.5px solid var(--color-border-default); border-radius: var(--radius-container); padding: var(--space-4); margin-top: var(--space-2); }
      ...
      <div className="wrap">
        <div className="inner">
          ...
          <div className="header">
            <div className="header-title">Skill Evaluator</div>
            <div className="header-sub">Upload your .skill file. Opus generates tailored evals, all evals run in parallel, Opus synthesises the final report. Run the Adherence Artifact first.</div>
          </div>

          {!skill && (
            <>
              {loadError && <div className="error-box">{loadError}</div>}
              {loading ? <div className="loading-msg">Reading .skill file…</div> : (
                <div className={`drop-zone${dragging ? " drag" : ""}`}
                  ...
                  <div className="drop-hint">.skill zip archive — run Adherence Artifact first</div>
          ...
          {skill && (
            <>
              <div className="skill-card">
                ...
                <div className="skill-meta">Ready for evaluation</div>
              </div>

              {!running && evals.length === 0 && (
              ...
              {allDone && totalTotal > 0 && (
                <div className="summary-bar">
                  <div>
                    <div className="summary-label">Pass Rate</div>
                    <div className="summary-detail">{totalPass} / {totalTotal} assertions</div>
                  </div>
                  <div className={`summary-score ${totalPass/totalTotal >= 0.9 ? "high" : totalPass/totalTotal >= 0.7 ? "mid" : "low"}`}>
                    {Math.round((totalPass/totalTotal)*100)}%
                  </div>
                </div>
              )}
              ...
              {allDone && (
                <button className="run-btn" onClick={generateReport} disabled={reportRunning}>
                  {reportRunning ? "Generating report…" : "Generate Report →"}
                </button>
              )}
              ...
              {reportMd && (
                <div className="report-card">
                  <div className="report-head">
                    <span className="out-label">Eval Report — paste back to Claude</span>
                    <button className={`copy-btn${copied ? " ok" : ""}`} onClick={copyReport}>
                      {copied ? "Copied ✓" : "Copy"}
                    </button>
                  </div>
                  <div className="md-block">{reportMd}</div>
                </div>
              )}

              <button className="run-btn ghost" style={{ marginTop: "0.5rem" }}
                onClick={() => { setSkill(null); setEvals([]); setReportMd(""); setError(""); }}>
...
function EvalCard({ ev }) {
  ...
  const gr = ev.graderResult;

  const badgeLabel = {
    idle:               "—",
    "running-executor": "Executing…",
    "running-grader":   "Grading…",
    done:  gr ? `${gr.summary?.passed}/${gr.summary?.total} passed` : "Done",
    error: "Error"
  }[ev.status] || "—";

  return (
    <div className={`eval-card ${ev.status}`}>
      <div className="eval-head" onClick={() => setOpen(o => !o)}>
        <div className="eval-name">{ev.eval_name || `eval-${ev.id}`}</div>
        <span className={`eval-badge ${ev.status}`}>{badgeLabel}</span>
      </div>
      {open && (
        <div className="eval-body">
          <div className="eval-prompt">{ev.prompt}</div>
          {gr?.expectations && (
            <div className="assert-list">
              {gr.expectations.map((a, i) => (
                <div key={i} className={`assert-row ${a.passed ? "pass" : "fail"}`}>
                  <span className="assert-icon">{a.passed ? "✓" : "✗"}</span>
                  <div>
                    <div>{a.text}</div>
                    {a.evidence && <div className="assert-evidence">{a.evidence}</div>}
                  </div>
                </div>
              ))}
            </div>
          )}
          {gr?.eval_feedback?.overall && (
            <div className="eval-feedback">{gr.eval_feedback.overall}</div>
          )}
          {ev.executorMd && (
            <>
              <div className="exec-toggle" onClick={() => setShowExec(s => !s)}>
                {showExec ? "▾ Hide transcript" : "▸ Show transcript"}
              </div>
              {showExec && <div className="exec-block">{ev.executorMd}</div>}
            </>
          )}
          {ev.status === "error" && (
            <div style={{ fontSize: "11px", color: "#e07070", marginTop: "8px" }}>
              {ev.graderResult?.error || ev.executorMd || "Unknown error"}
            </div>
          )}
        </div>
      )}
